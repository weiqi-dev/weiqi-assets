import{l as e}from"./FavoriteService-Vp2l_PCI.js";import{t}from"./GameResult-BMILmLn5.js";import{t as n}from"./adapters-CwjzkNpn.js";import{n as r}from"./JosekiThumbnail-C22KvNDm.js";var i=class{get title(){return this._title}constructor(t){e(this,`_title`,`棋谱列表`),e(this,`readMarkService`,void 0),e(this,`logger`,void 0),e(this,`category`,void 0),e(this,`providers`,void 0),e(this,`gameService`,void 0),e(this,`gameHistoryStorage`,void 0),e(this,`card`,void 0),e(this,`toast`,void 0),e(this,`games`,[]),e(this,`readMarkIds`,[]),e(this,`currentUserId`,void 0),e(this,`onNavigate`,void 0),e(this,`onClearReadMarks`,void 0),e(this,`initialized`,!1),this.readMarkService=t.readMarkService,this.logger=t.logger,this.category=t.category,this.providers=t.providers,this.gameService=t.gameService,this.gameHistoryStorage=t.gameHistoryStorage,this.onNavigate=t.onNavigate,this.onClearReadMarks=t.onClearReadMarks,this.card=n.createCard(),this.toast=n.createToast()}async initialize(){var e=this;e.initialized||(e.initialized=!0)}handleParams(e){if(e.gamesJson)try{this.games=JSON.parse(e.gamesJson)}catch(e){this.logger.warn(`解析棋谱数据失败`),this.games=[]}e.title&&(this._title=e.title),e.userId&&(this.currentUserId=e.userId),this.loadReadMarks(),this.render()}async loadGames(e,t,n){var r=this;r.currentUserId=n;let i=r.providers.get(e);if(!i){r.logger.error(`未找到 category="${e}" 的棋谱列表提供者`),r.toast.error(`未找到数据提供者`),r.games=[],r.render();return}try{let e=await i.getGameArchiveIds(t),n=[];for(let t of e)if(r.gameHistoryStorage){let e=await r.gameHistoryStorage.findById(t);if(e&&e.metadata){let t=e.metadata.result||void 0,r=e.metadata.blackName||e.metadata.black||``,i=e.metadata.whiteName||e.metadata.white||``;n.push({id:e.gameId,archiveId:e.id,black:r,white:i,date:e.metadata.date||``,...t?{result:t}:{}})}}else{let e=t.split(`-`).slice(1).join(`-`)||t;n.push({id:e,archiveId:t,black:`-`,white:`-`,date:`-`})}r.games=n,r.loadReadMarks(),r.render()}catch(e){r.logger.error(`加载棋谱数据失败`,e),r.toast.error(`加载失败`),r.games=[],r.render()}}setData(e){this.games=e,this.loadReadMarks(),this.render()}async loadReadMarks(){var e=this;e.readMarkIds=await e.readMarkService.getReadMarks(e.category)}async clearReadMarks(){var e=this,t;await e.readMarkService.clearReadMarks(e.category),e.readMarkIds=[],(t=e.onClearReadMarks)==null||t.call(e),e.render()}async onItemClick(e){var t=this;await t.readMarkService.markRead(t.category,e),t.readMarkIds.push(e);let n=t.games.find(t=>t.id===e);!n||!n.archiveId||t.onNavigate&&t.onNavigate(`replay`,{archiveId:n.archiveId})}render(){let e=document.getElementById(`page-root`);if(!e)return;let n=`📋 共 ${this.games.length} 局，已读 ${this.games.filter(e=>this.readMarkIds.includes(e.id)).length} 局`,r=`<div class="source-header-controls">
      ...
      <div class="source-group">
        ...
        <div class="game-list">
          ${this.games.map((e,n)=>{let r=this.readMarkIds.includes(e.id),i=e.black===this.currentUserId,a=e.white===this.currentUserId,o=t(e.result),s=``;return o.includes(`黑`)?s=i?`result-win`:`result-lose`:o.includes(`白`)&&(s=a?`result-win`:`result-lose`),`
        <div class="game-card-wrapper ${r?`viewed`:``}" data-index="${n}" data-id="${e.id}">
          <div class="game-card">
    ...
    `,e.querySelectorAll(`.game-card-wrapper`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.id;t&&this.onItemClick(t)})}),this.bindClearBtn()}bindClearBtn(){let e=document.getElementById(`clearReadBtn`);e&&(e.onclick=()=>this.clearReadMarks())}destroy(){this.card.destroy(),this.toast.destroy(),this.games=[],this.readMarkIds=[],this.initialized=!1}},a=class{constructor(){e(this,`currentFilter`,`all`)}getPatternTags(e){let t=[];return e.prefixLen>=8&&e.frequency>=5&&t.push(`hot`),e.prefixLen>=8&&e.prefixLen>=e.totalMoves&&t.push(`hit`),e.prefixLen>=12&&t.push(`complex`),t}setFilter(e){this.currentFilter=e}getFilter(){return this.currentFilter}filterPatterns(e){return e.filter(e=>this.currentFilter===`all`?!0:this.getPatternTags(e).includes(this.currentFilter))}getCounts(e){return{all:e.length,hot:e.filter(e=>this.getPatternTags(e).includes(`hot`)).length,hit:e.filter(e=>this.getPatternTags(e).includes(`hit`)).length,complex:e.filter(e=>this.getPatternTags(e).includes(`complex`)).length}}},o=class{constructor(t){e(this,`readMarkService`,void 0),e(this,`readMarkIds`,[]),e(this,`currentCategory`,void 0),this.readMarkService=t}setCategory(e){this.currentCategory=e}async loadReadMarks(){var e=this;e.currentCategory&&(e.readMarkIds=await e.readMarkService.getReadMarks(e.currentCategory))}async markRead(e){var t=this;t.currentCategory&&(await t.readMarkService.markRead(t.currentCategory,e),t.readMarkIds.includes(e)||t.readMarkIds.push(e))}async clearReadMarks(){var e=this;e.currentCategory&&(await e.readMarkService.clearReadMarks(e.currentCategory),e.readMarkIds=[])}isRead(e){return this.readMarkIds.includes(e)}getReadMarkIds(){return this.readMarkIds}reset(){this.readMarkIds=[],this.currentCategory=void 0}},s=class{showWinrateDetail(e){if(!(e!=null&&e.delta))return;let t=e.delta,n=`<div class="winrate-detail-row"><span class="winrate-detail-label">胜率变化</span><span class="winrate-detail-value">`+(t>0?`+`:``)+(t*100).toFixed(2)+`%</span></div>`;if(e.stddev!==void 0){let t=e.stddev,r=`较稳定`;r=t<.02?`很稳定`:t<.04?`较稳定`:t<.06?`一般`:`不稳定`,n+=`<div class="winrate-detail-row"><span class="winrate-detail-label">标准差</span><span class="winrate-detail-value">`+t.toFixed(4)+` (`+r+`)</span></div>`}if(e.samples!==void 0){let t=e.samples,r=e.positive||0,i=e.negative||0,a=e.neutral||0,o=t>0?Math.round(r/t*100):0,s=t>0?Math.round(i/t*100):0,c=t>0?Math.round(a/t*100):0;n+=`<div class="winrate-detail-row"><span class="winrate-detail-label">样本数</span><span class="winrate-detail-value">`+t+`</span></div>`,n+=`<div class="winrate-bar"><div class="winrate-bar-positive" style="width:`+o+`%"></div><div class="winrate-bar-neutral" style="width:`+c+`%"></div><div class="winrate-bar-negative" style="width:`+s+`%"></div></div>`,n+=`<div class="winrate-bar-labels"><span>黑有利 `+r+` (`+o+`%)</span><span>中性 `+a+`</span><span>白有利 `+i+` (`+s+`%)</span></div>`}n+=`<div class="winrate-hint">💡 delta > 0: 先手有利</div>`;let r=document.getElementById(`winrate-content`);r&&(r.innerHTML=n);let i=document.getElementById(`winrate-backdrop`),a=document.getElementById(`winrate-sheet`);i&&i.classList.add(`active`),a&&a.classList.add(`active`)}formatWinrate(e){return e===void 0?{display:`-`,className:`neutral`}:{display:(e>0?`+`:``)+(e*100).toFixed(1)+`%`,className:e>.01?`positive`:e<-.01?`negative`:`neutral`}}},c=class{constructor(t){e(this,`config`,void 0),this.config=t}renderFilterTabs(e,t){let n=document.getElementById(`filter-tabs`);if(!n)return;let r=this.config.filterManager.getCounts(e);n.innerHTML=[{id:`all`,label:`全部`,count:r.all},{id:`hot`,label:`🔥热门`,count:r.hot},{id:`hit`,label:`🎯命中`,count:r.hit},{id:`complex`,label:`🧩复杂`,count:r.complex}].map(e=>`
