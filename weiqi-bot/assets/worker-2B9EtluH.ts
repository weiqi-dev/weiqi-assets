...
async function initBackend(baseUrl?: string): Promise<void> {
  ...
  try {
    ...
    // 使用相对路径，确保在所有环境（浏览器、app webview）都能正常工作
    // Worker 在 assets/worker.js，tfjs 在 ../tfjs/
    const wasmBase = baseUrl ? baseUrl + 'tfjs/' : '../tfjs/';
    debugLog('log', 'WASM base path:', wasmBase);
