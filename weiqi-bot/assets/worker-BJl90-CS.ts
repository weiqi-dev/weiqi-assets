...
// 在导入 TensorFlow.js 之前设置 WASM 路径
// Worker 文件在 assets/worker.js，WASM 文件在 ../tfjs/
// 所以需要使用相对路径 ../tfjs/
setWasmPaths('../tfjs/');
...
/**
 * Create empty board input for warm-up
 * @param outSpatial - Output spatial tensor
 * @param outGlobal - Output global tensor
 */
function createEmptyInput(outSpatial: Float32Array, outGlobal: Float32Array): void {
  // Empty board (all zeros)
  const emptyBoard = new Uint8Array(BOARD_AREA);
  
  // Fill inputs for empty board, black to play
  fillInputsV7Fast({
    stones: emptyBoard,
    koPoint: -1,
    currentPlayer: 'black',
...
async function initBackend(baseUrl?: string): Promise<void> {
  debugLog('log', 'Initializing backend', { baseUrl });
  
  // Try WebGL first (best browser support and performance)
  try {
...
async function ensureBackend(baseUrl?: string): Promise<void> {
  // 如果 baseUrl 变化，需要重新初始化
  if (backendPromise && loadedBaseUrl !== baseUrl) {
    debugLog('warn', 'baseUrl changed, reinitializing backend', { old: loadedBaseUrl, new: baseUrl });
    backendPromise = null;
    loadedBaseUrl = undefined;
  }
  if (!backendPromise) {
    debugLog('log', 'Creating new backend promise', { baseUrl });
    loadedBaseUrl = baseUrl;
    backendPromise = initBackend(baseUrl)
      .then(() => {
        tf.enableProdMode();
...
async function ensureModel(modelUrl: string): Promise<void> {
  ...
  if (res.body) {
    ...
    // fallback: 如果不支持 ReadableStream，使用 XMLHttpRequest 支持进度
    debugLog('log', 'ReadableStream not supported, using XMLHttpRequest fallback', { modelUrl });
    ...
    const buf = await new Promise<Uint8Array>((resolve, reject) => {
      ...
      xhr.onprogress = (e) => {
        if (e.lengthComputable) {
          ...
          // 如果不知道总大小，按 1MB 计算
          const progress = (e.loaded / 1000000) * 100;
          post({
            type: 'katago:progress',
            loaded: e.loaded,
            total: 0,
      ...
      xhr.onload = () => {
        if (xhr.status === 200) {
          ...
          reject(new Error(`Failed to fetch model: ${xhr.status} ${xhr.statusText}`));
        }
      };
      
      xhr.onerror = () => reject(new Error('Network error during model fetch'));
      xhr.ontimeout = () => reject(new Error('Timeout during model fetch'));
    ...
    search = null;
    searchKey = null;
    debugLog('log', 'Model loaded successfully (XHR fallback)', { modelName: loadedModelName, modelUrl });
  }
}
...
async function handleMessage(msg: KataGoWorkerRequest): Promise<void> {
  ...
  if (msg.type === 'katago:init') {
    ...
    try {
      // 1. 加载后端和模型
      await ensureBackend(msg.baseUrl);
      ...
      // 2. 执行 warm up（默认 true）
      if (msg.warmUp !== false) {
        debugLog('log', 'Starting warm up (3 iterations)...');
        
        const warmUpStart = performance.now();
        
        // 准备空棋盘输入
        const emptySpatial = new Float32Array(V7_SPATIAL_STRIDE);
        const emptyGlobal = new Float32Array(V7_GLOBAL_STRIDE);
        createEmptyInput(emptySpatial, emptyGlobal);
        
        // 执行 3 次推理，等待 GPU 初始化完成
        for (let i = 0; i < 3; i++) {
          const iterStart = performance.now();
          
          // 创建输入张量
          const spatial = tf.tensor4d(emptySpatial, [1, BOARD_SIZE, BOARD_SIZE, 22]);
          const global = tf.tensor2d(emptyGlobal, [1, 19]);
          
          // 执行推理
          const out = model!.forwardValueOnly(spatial as tf.Tensor4D, global as tf.Tensor2D);
          await Promise.all([out.value.data(), out.scoreValue.data()]);
          ...
          debugLog('log', `Warm up ${i + 1}/3 completed in ${duration.toFixed(0)}ms`);
        }
        
        const totalDuration = performance.now() - warmUpStart;
        debugLog('log', `Warm up completed in ${totalDuration.toFixed(0)}ms`);
      } else {
        debugLog('log', 'Warm up skipped (warmUp = false)');
      }
